...
class Particle():
    ...
    def updateVelocities(stopEvent):
        while not stopEvent.is_set():
            ...
            newVels = defaultdict(lambda: np.array([]))
            for p in Constants.PARTICLES:
                ...
                if len(neighbors) > 1: # Self is found as a 'neighbor', ensure one other particle is present
                    ...
                    forces = np.array([influences[i] * math.exp(-distances[i] / Particle.MAX_INFLUENCE_DIST) for i in range(len(neighborPositions))])
